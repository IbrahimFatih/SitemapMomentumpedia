...
";
        s += titleCode(entry);
        s += "<span style='font-size:80%'>" + labelsCode(entry);
...
function showHeaderOption(){
    ...
    s += "
<tr>";
    s += "<td style='text-align:right'>Semoga : ";
    s += "<td><select onchange='sortBy(this.value.substr(1), this.value.substr(0,1))'>";
    s += "<option value='0published'/>semua pos";
    s += "<option value='1title'/>";
    s += "</select>";
    s += "
<tr>";
    s += "<td style='text-align:right'>Bermanfaat : ";
    s += "<td><select onchange='showPostsWLabel(this.value)' id='cl_Carilabel'>";
    s += "<option value='*'/>blog ini";
...
function publishedDateCode(entry){
    ...
    var s = "<a href='javascript:showPostsInDate(\""+m+"\")' title='Sort article by "+m+"'>" + m + "</a>/";
    return s;
}
